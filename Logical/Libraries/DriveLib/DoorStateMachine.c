...
void DoorStateMachine(struct DoorStateMachine* inst)
{
	...
	switch(inst->state){
		...
		case ST_CLOSE:
			{
				inst->speed = 0;
				if(inst->direction==1 && inst->sw4 == 0)
		...
		case ST_ACC_POS:
			{
				inst->speed = 800;
				if(inst->direction==1 && inst->sw3 == 0)
					inst->state=ST_POS;
				if(inst->direction==0)
		...
		case ST_POS:
			{
				inst->speed = 450;
				if(inst->direction==1 && inst->sw2==0)
		...
		case ST_DEC_POS:
			{
				inst->speed = 150;
				if(inst->direction==1 && inst->sw1 == 0)
					inst->state=ST_OPEN;
		...
		case ST_OPEN:
			{
				inst->speed = 0;
				if(inst->direction==0 && inst->sw1==1)
		...
		case ST_ACC_NEG:
			{
				inst->speed = -800;
				if(inst->direction==0 && inst->sw2==1 )
					inst->state=ST_NEG;
				if(inst->direction==1)
					inst->state=ST_DEC_POS;
				break;
		...
		case ST_NEG:
			{
				inst->speed = -450;
				if(inst->direction==0 && inst->sw3==1)
					inst->state=ST_DEC_NEG;
				if(inst->direction==1)
					inst->state=ST_POS;
				break;
		...
		case ST_DEC_NEG:
			{
				inst->speed = -150;
				if(inst->direction==0 && inst->sw4 == 1)
					inst->state=ST_CLOSE;
				break;
			}
	}
